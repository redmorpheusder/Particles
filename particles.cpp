...
const char *WINDOWTITLE = { "Particle System -- Altynbek Yermurat" };
...
struct fire
{
	...
	float vxb, vyb, vzb; //Reset velocity
...
Animate( void )
{
	...
	for( i=0, p=Fire; i < NUMFIRES; i++, p++ )
	{
		dt = Time - p->t0;
		p->x = (p->vy * dt) + p->y0;
		p->y = ((p->vz * dt) + 0.8 * GRAVITY * (dt * dt)) + p->z0;
		p->z = (p->vx * dt) + p->x0;
...
InitGraphics( void )
{
	...
	for( i=0, p=Fire; i < NUMFIRES; i++, p++ )
	{
		...
		double x_rot_y = (tan(th2) - sin(th2));
		double z_rot_y = (cos(th2) + cos(th2));

		p->vxb = (v * tan(th));
		p->vxb = p->vxb > 0 ? p->vxb : -p->vxb;
		p->vxb *= x_rot_y;

		p->vzb = (v * sin(th));
		p->vzb = p->vzb > 0 ? p->vzb : -p->vzb;
		p->vzb *= z_rot_y;
